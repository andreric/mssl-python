...
class MSSLClassifierIPCW(Method):
    """
    ...
    def __init__(self, lambda_1=0.1, lambda_2=0):
        ...
        super().__init__('MSSLClassifierIPCW', 'MTL')
    ...
    def fit(self, x, y, intercept=True, **kwargs):
        ...
        x, y, d, v, offsets = self.__preprocess_data(x, y, d, v)
        self.offsets = offsets

        W, Omega = self.__mssl_train(x, y, d, v)
        self.W = W.copy()
    ...
    def __preprocess_data(self, x, y, d, v):
        ...
        for t in range(self.ntasks):
            ...
            d[t] = d[t].astype(np.int8).ravel()
            v[t] = v[t].astype(np.float32).ravel()
            if len(y[t].shape) < 2:
                y[t] = y[t][:, np.newaxis]
    ...
    def __mssl_train(self, x, y, d, v):
        ...
        for t in range(len(x)):
            w_t = inverse_probability_of_censoring_weights(v[t], d[t])
            weights.append(w_t)
